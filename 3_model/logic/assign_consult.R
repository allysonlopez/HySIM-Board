consult_group_probability_columns <- c(
...
assign_consult_need_and_group <- function(consult_probability, triage_priority) {
  ...
  needs_consult <- stats::rbinom(
    n = 1,
    size = 1,
    prob = consult_probability_value
  )
  ...
  group_probabilities <- normalize_probabilities(
    unlist(matching_row[consult_group_probability_columns], use.names = FALSE)
  )
  
  consult_group_code <- sample(
    seq_along(group_probabilities),
    size = 1,
    prob = group_probabilities
  )
  
  c(
    needs_consult = 1,
    consult_group_code = consult_group_code
  )
}